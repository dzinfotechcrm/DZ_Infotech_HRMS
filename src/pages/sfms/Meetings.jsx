...
export default function Meetings() {
  ...
  const loading = meetingsLoading || leadsLoading || teamsLoading;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const payload = {
        ...data,
        team_id: selectedLead?.team_id || null,
      ...
      if (selectedLead && selectedLead.stage === 'Meeting Scheduled') {
        ...
        await supabase.from('sfms_lead_timeline').insert([{ lead_id: data.lead_id, stage: 'Meeting Completed', changed_by: 'System (Meeting Logged)' }]);
      }
  ...
  const enrichedMeetings = useMemo(() => {
    if (loading) return [];
    return [...meetings].sort((a, b) => new Date(b.meeting_date) - new Date(a.meeting_date)).map(m => {
  ...
  const leadOptions = leads.map(l => ({ value: l.id, label: l.company_name }));
            ...
            <tbody className="divide-y divide-neutral-50">
              {enrichedMeetings.map(meeting => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium text-neutral-700">{meeting.team_name}</td>
