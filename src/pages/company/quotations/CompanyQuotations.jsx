...
export default function CompanyQuotations() {
  ...
  const { items: savedQuotations = [], loading } = useSupabaseCollection('quotations');
  ...
  useEffect(() => {
    if (!loading) {
      setFormData(prev => ({
        ...prev,
        quotationNumber: getInitialState(savedQuotations).quotationNumber
      }));
    }
  }, [savedQuotations, loading]);
  ...
  const handleGenerateAndSave = async () => {
    ...
    try {
      ...
      // 2. Save to Supabase
      const payload = { ...formData, id: Date.now().toString(), created_at: new Date().toISOString() };
...
*DZ Infotech*`;
  ...
  const handleDelete = async () => {
    if (deleteItem) {
      try {
        const { error } = await supabase.from('quotations').delete().eq('id', deleteItem.id);
